...
public class SpriteBattling : MonoBehaviour {
    ...
    public void walk(float time, Vector3 destination)
    {        
        nowDestination = destination;
        gameObject.GetComponent<Rigidbody2D>().velocity = (new Vector2 (destination.x, destination.y) - new Vector2(gameObject.transform.position.x, gameObject.transform.position.y)) / time;
        isWalk = true;
